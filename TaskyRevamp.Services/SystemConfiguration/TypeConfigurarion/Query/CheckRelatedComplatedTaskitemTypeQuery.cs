...
namespace TaskyRevamp.Services.SystemConfiguration.PriorityConfiguration.Query
{
    ...
    public class CheckRelatedComplatedTaskitemTypeHandler : IRequestHandler<CheckRelatedComplatedTaskitemTypeQuery, bool>
    {
        ...
        {
            var deleted = await _TypeRepository.FindBy(k => k.NameArabic == request.TypeDto.NameArabic || k.NameEnglish == request.TypeDto.NameEnglish && k.IsDeleted);

            var olddata = deleted.Value.FirstOrDefault();
            if (olddata != null)
            {
                var data = await _TaskItemRepository.FindBy(k => k.TaskTypeId == olddata.Id && k.StatusId == Guid.Parse("C8D504C7-9402-4F91-223C-08DE3318A61C"));
