...
namespace TaskyRevamp.Client.Consumer
{
    public class TaskCommentConsumer
    {
        ...
        public async Task<CommonApiResponse<bool>> UpdateTaskComment(TaskCommentDto taskCommentDto)
        {
            var url = $"api/TaskComment/UpdateTaskComment";
            var res = await _taskyService.PostJsonAsync<CommonApiResponse<bool>>(url, taskCommentDto);

            return res.Data;
        }
        public async Task<CommonApiResponse<bool>> DeleteTaskComment(Guid id)
        {
            var url = $"api/TaskComment/{id.ToString()}";
