...
public class FileManagement : IFileManagement
{
    ...
    public async Task<List<FileDto>> GetFilesInfoInternal(List<Guid> fileIds)
    {
        ...
        var files = await context.FileMapping.Where(x => fileIds.Contains(x.FileId)).ToListAsync();
    ...
    {
        FileDto answer = null;
        var file = context.FileMapping.FirstOrDefault(x => x.FileId == fileId);
        if (file != null)
        {
            var fileExtension = Path.GetExtension(file.FileName).ToLower();
    ...
    public async Task<Guid> UploadFile(List<byte> bytes, string fileName, FileType fileType, string extentions, Guid? fileId = null)
    {
        var byteArray = bytes.ToArray();
        return await UploadFile(byteArray, fileName, fileType, extentions, fileId);
    }

    public async Task<Guid> UploadFile(string base64, string fileName, FileType fileType, string extentions, Guid? fileId = null)
    {
        ...
        return await UploadFile(bytes, fileName, fileType, extentions, fileId);
    }

    public async Task<Guid> UploadFile(byte[] bytes, string fileName, FileType fileType, string extensions, Guid? fileId = null)
    {
        return await Impersonate(() => UploadFileInternal(bytes, fileName, fileType, extensions, fileId));
    }

    private async Task<Guid> UploadFileInternal(byte[] bytes, string fileName, FileType fileType, string extensions, Guid? fileId = null)

    {
        ...
        FileMapping file = new FileMapping()
        {
            Id = Guid.NewGuid(),
            FileId = newFileId,
            FileName = fileName,
    ...
    public async Task<Guid> DeleteFile(Guid fileId, FileType fileType)
    {
        ...
        if (file is not null)
        {
            context.FileMapping.Remove(file);
    ...
    {
        var files = context.FileMapping.ToList();
        if (files != null)
        {
